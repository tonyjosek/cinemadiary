...
export const MovieDetail = () => {
  ...
  return (
    <main>
      <section className=" text-gray-700 flex flex-wrap justify-around py-5">
        ...
        <p className="py3">{movie.overview}</p>
        {movie.genres ? (<p className="my-7 flex flex-wrap gap-2" >
          { movie.genres.map((genre) =>(
          <span className="p-2 mr-2 border border-grey-200 rounded" key={genre.id}>{genre.name}</span>
          ))}          
        </p>) :""}      
        ...
        <div className="flex items-center">
            ...
            <p className="ml-1 text-sm font-bold text-gray-700 dark:text-white">{movie.vote_average}</p>
